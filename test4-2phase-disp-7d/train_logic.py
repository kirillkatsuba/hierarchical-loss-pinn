...
def get_plot_data(pwat=1.5, poil=2.0, kwat=1.5, koil = 0.3):
    ...
    cache_name = (
        f"test4_pwat{pwat:g}_poil{poil:g}_kwat{kwat:g}_koil{koil:g}".replace(".", "p")
        + ".npz"
    )
...
def plot_validation(
    ...
    ax = axes[0, 0]
    im = ax.imshow(np.squeeze(swat[:, :, :, -1]))
    ...
    ax = axes[0, 1]
    im = ax.imshow(np.squeeze(model_prediction[:, 2].reshape(nx0, nx1, nx2)))
    ...
    ax = axes[0, 2]
    im = ax.imshow(np.squeeze(pres[:, :, :, -1]))
    ...
    # 1,0 – Pressure, PINN
    ax = axes[1, 0]
    im = ax.imshow(np.squeeze(model_prediction[:, 0].reshape(nx0, nx1, nx2)))
    ...
    ax.scatter(
        ...
        np.squeeze(model_prediction[:, 2].reshape(nx0, nx1, nx2))[:, 0],
        label='PINN'
    )
    ax.grid()
    ax.set_xlabel('x')
    ...
    ax.scatter(
        np.linspace(0.0, 1.0, nx0),
        np.squeeze(model_prediction[:, 0].reshape(nx0, nx1, nx2))[:, 0],
        label='PINN'
    )
    ax.grid()
    ax.set_xlabel('x')
