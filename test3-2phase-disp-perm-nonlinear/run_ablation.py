...
def parse_args():
    parser = argparse.ArgumentParser(description="Run ablation for test3.")
    parser.add_argument(
        "--balancers",
        nargs="+",
    ...
    parser.add_argument("--batch-size", type=int, default=150, help="Batch size.")
    parser.add_argument("--lr", type=float, default=3e-3, help="Learning rate.")
    parser.add_argument("--hidden-dim", type=int, default=128, help="Hidden width.")
...
def evaluate_test3(module, checkpoint_path, device, test_n_collocation, hidden_dim, num_layers):
    ...
    simulation_data, points, _, _, _, _ = module.prepare_training_data(
        base_dir=base_dir,
...
def main():
    ...
    module = load_module(script_path, "test3_train_pinn")
    device = torch.device(args.device)
    rows = []

    if args.prepare_validation_gt and args.plot_every > 0:
        print("Precomputing validation GT cache for test3...")
    ...
    for n_collocation in args.n_values:
        for balancer in args.balancers:
            ...
            prefix = f"test3_metrics_{balancer}_n{n_collocation}"
